...
当前数据库已经按照 [market.sql](../../sql/market.sql) 建立完成，因此本项目书中所有数据库设计、数据表概要、接口示例与业务流程均以现有数据库结构为准，不再保留“建议型数据库设计”表述。
...
- 关系型数据库适合本系统的业务模型
- 已依据 [market.sql](../../sql/market.sql) 完成建表
...
本系统数据库名称为 `supermarket_inventory`，字符集为 `utf8mb4`，排序规则为 `utf8mb4_unicode_ci`。数据库脚本文件为 [market.sql](../../sql/market.sql)。
