# 登录页迁移 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将 `system/pages/登录页.html` 完整迁移至 Vue 3 前端项目，1:1 复刻深蓝科技风 UI，对接后端账号密码登录接口。

**Architecture:** 重写 `LoginView.vue` 为独立组件，所有样式通过 `<style scoped>` 内聚，图片资源复制到 `src/assets/`，Google Fonts 在 `index.html` 全局加载。登录流程复用现有 `authStore` 和 `api/auth.js`，验证码为前端临时方案。

**Tech Stack:** Vue 3 Composition API, Pinia, Vue Router, Axios, CSS (scoped, no UI framework)

---

## File Structure

- Copy: `system/pages/back_login.png` → `system/frontend/src/assets/back_login.png` — 背景图
- Copy: `system/pages/logo_A.png` → `system/frontend/src/assets/logo_A.png` — Logo
- Modify: `system/frontend/index.html` — 添加 Google Fonts `<link>` 标签
- Rewrite: `system/frontend/src/views/login/LoginView.vue` — 登录页主组件（模板 + 脚本 + 样式）

不变的文件（已就绪，直接复用）：
- `system/frontend/src/api/auth.js` — `login(data)` 调用 `POST /auth/login`
- `system/frontend/src/stores/auth.js` — `authStore.login(payload)` 存储 token/user
- `system/frontend/src/router/index.js` — `/login` 路由，`meta: { public: true }`
- `system/frontend/src/api/request.js` — axios 实例，响应拦截器自动解包 `code===0`
...
**Files:**
- Copy: `system/pages/back_login.png` → `system/frontend/src/assets/back_login.png`
- Copy: `system/pages/logo_A.png` → `system/frontend/src/assets/logo_A.png`

- [ ] **Step 1: 复制背景图**

```bash
cp "E:\Supermarket\system\pages\back_login.png" "E:\Supermarket\system\frontend\src\assets\back_login.png"
```

- [ ] **Step 2: 复制 Logo**

```bash
cp "E:\Supermarket\system\pages\logo_A.png" "E:\Supermarket\system\frontend\src\assets\logo_A.png"
```

- [ ] **Step 3: 验证文件存在**

```bash
ls "E:\Supermarket/system/frontend/src/assets/back_login.png" "E:\Supermarket/system/frontend/src/assets/logo_A.png"
```

Expected: 两个文件路径均显示，无报错。

- [ ] **Step 4: Commit**

```bash
git add system/frontend/src/assets/back_login.png system/frontend/src/assets/logo_A.png
git commit -m "feat(login): 复制登录页背景图和Logo到前端assets"
```

---

### Task 2: 添加 Google Fonts

**Files:**
- Modify: `system/frontend/index.html`

- [ ] **Step 1: 在 `<head>` 中添加 Google Fonts 链接**

在 `system/frontend/index.html` 的 `<head>` 标签内，`<title>` 之前添加以下内容：
...
<html lang="zh-CN">
  <head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+SC:wght@300;400;500;600;700&family=Orbitron:wght@500;600;700&display=swap" rel="stylesheet">
...
```bash
git add system/frontend/index.html
git commit -m "feat(login): 添加 Google Fonts（Noto Sans SC + Orbitron）"
```

---

### Task 3: 重写 LoginView.vue — 模板与脚本

**Files:**
- Rewrite: `system/frontend/src/views/login/LoginView.vue`

- [ ] **Step 1: 编写完整的 LoginView.vue 组件**

完整替换 `system/frontend/src/views/login/LoginView.vue`，内容如下：
...
在 IDE 中检查 `LoginView.vue` 无红色波浪线报错（Vue 语法、import 路径正确）。

- [ ] **Step 3: Commit**

```bash
git add system/frontend/src/views/login/LoginView.vue
git commit -m "feat(login): 重写登录页组件，1:1复刻HTML深蓝科技风UI"
```

---

### Task 4: 添加登录页 scoped 样式

**Files:**
- Modify: `system/frontend/src/views/login/LoginView.vue`（在 `</script>` 后追加 `<style scoped>`）

- [ ] **Step 1: 在 LoginView.vue 末尾追加 scoped 样式**

在 `</script>` 标签之后，文件末尾追加以下内容：
...
在 IDE 中检查 `<style scoped>` 内无红色波浪线。

- [ ] **Step 3: Commit**

```bash
git add system/frontend/src/views/login/LoginView.vue
git commit -m "feat(login): 添加登录页深蓝科技风 scoped 样式"
```
...
```bash
cd "E:\Supermarket\system\frontend" && npm run dev
```

Expected: Vite 启动成功，显示本地地址（如 `http://localhost:5173`）。

- [ ] **Step 2: 浏览器访问登录页**

打开 `http://localhost:5173/login`，验证：
- 页面整体缩放 90%
- 深蓝背景图正常显示
- 顶部导航栏 Logo + 品牌名正常
- 左侧宣传文案正常（标题渐变色、描述文字）
...
1. 确认默认值 admin/admin123 已预填
2. 输入验证码，点击登录 → 验证成功跳转到 `/dashboard`
3. 清空用户名，点击登录 → 显示"请输入用户名"
4. 清空密码，点击登录 → 显示"请输入密码"
5. 清空验证码，点击登录 → 显示"请输入验证码"
6. 输入错误验证码 → 显示"验证码错误"，验证码刷新

- [ ] **Step 4: 测试非功能区域**

1. 点击"短信登录" Tab → 显示"暂未开放" toast
2. 点击"获取验证码" → 显示"暂未开放" toast
3. 点击微信/钉钉/企业账户按钮 → 各显示"暂未开放" toast
4. 点击"忘记密码" → 显示"暂未开放" toast
5. 点击"联系管理员开通" → 显示"暂未开放" toast
6. 页脚版权信息正常显示

- [ ] **Step 5: 测试响应式布局**

1. 缩小浏览器窗口至 1024px 以下 → 左侧 hero 隐藏，卡片居中
2. 缩小至 640px 以下 → 移动端布局，背景切换为纯渐变
...
```bash
git add -A
git commit -m "fix(login): 修复登录页迁移验证中发现的问题"
```
