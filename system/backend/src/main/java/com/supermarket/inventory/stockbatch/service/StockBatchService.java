...
public class StockBatchService {
    ...
    public StockBatchService(StockBatchMapper stockBatchMapper, StockService stockService) {
    ...
    public void lock(Long skuId, Long batchId) {
        ...
        stockBatchMapper.updateStatus(batchId, skuId, STATUS_LOCKED);
    ...
    public void unlock(Long skuId, Long batchId) {
        ...
        stockBatchMapper.updateStatus(batchId, skuId, targetStatus);
    ...
    public void damage(Long skuId, Long batchId, StockBatchDamageRequest request) {
        ...
        stockService.damage(skuId, damageQuantity);
    ...
    @Transactional
    public void consumeAvailableBatches(Long skuId, int quantity, String sourceType, Long sourceId) {
        validateConsumeRequest(skuId, quantity, sourceType, sourceId);
        List<StockBatch> batches = stockBatchMapper.findAvailableBatchesForConsumption(skuId);
        int availableQuantity = batches.stream()
                .mapToInt(batch -> batch.getQuantity() == null ? 0 : batch.getQuantity())
                .sum();
        if (availableQuantity < quantity) {
            throw new BusinessException("可用批次数量不足");
        }
        ...
        for (StockBatch batch : batches) {
            ...
            int beforeQuantity = batch.getQuantity() == null ? 0 : batch.getQuantity();
            int consumeQuantity = Math.min(beforeQuantity, remaining);
            int afterQuantity = beforeQuantity - consumeQuantity;
            String afterStatus = afterQuantity == 0 ? STATUS_DEPLETED : STATUS_AVAILABLE;
            ensureBatchUpdated(stockBatchMapper.updateRemainingQuantityAndStatus(
                    batch.getId(), batch.getSkuId(), afterQuantity, afterStatus));
            writeOutboundLog(batch, -consumeQuantity, afterQuantity, sourceType, sourceId);
    ...
    public void close(Long skuId, Long batchId) {
        ...
        stockBatchMapper.updateStatus(batchId, skuId, STATUS_CLOSED);
    ...
    public int markExpiredBatches(LocalDate today) {
        ...
        for (StockBatch batch : batches) {
            stockBatchMapper.updateStatus(batch.getId(), batch.getSkuId(), STATUS_EXPIRED);
    ...
    private void writeOutboundLog(StockBatch batch, int changeQuantity, int afterQuantity,
                                  String sourceType, Long sourceId) {
        StockBatchLog log = baseLog(batch, CHANGE_TYPE_OUTBOUND, sourceType);
        log.setChangeQuantity(changeQuantity);
        log.setBeforeQuantity(batch.getQuantity());
        log.setAfterQuantity(afterQuantity);
    ...
    private void validateConsumeRequest(Long skuId, int quantity, String sourceType, Long sourceId) {
        if (skuId == null) {
            throw new BusinessException("SKU ID不能为空");
        }
        if (quantity <= 0) {
        ...
        if (sourceType == null || sourceType.trim().isEmpty()) {
            throw new BusinessException("出库来源类型不能为空");
        }
        if (sourceId == null) {
            throw new BusinessException("出库来源ID不能为空");
        }
    }
